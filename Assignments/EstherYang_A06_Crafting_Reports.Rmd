...
summary_Ozone <- epa_data %>% 
  ...
  arrange(mean_Ozone)
knitr::kable(summary_PM25, caption = "Mean Particulates (2.5mm)", digits = 2, col.names = c("County", "$\\mu g/m^3$"))
knitr::kable(summary_Ozone, caption = "Mean Ozone", digits = 2, col.names = c("County", "$\\mu g/m^3$"))
...
```{r ozone2018, echo=FALSE, fig.align='left', fig.height=3, fig.cap="2018 monthly Ozone levels", message=FALSE, warning=FALSE, error=FALSE}
epa_data %>% 
  filter(year(Date) == 2018) %>% 
  ggplot(aes(x = month(Date, label = TRUE), y = Ozone)) +
  geom_boxplot(fill = "pink") +
  labs(title = "Monthly Ozone concentrations (2018)", x = "Month", y = expression(Concentration~(mu*g/m^3))) +
  theme_minimal()
...
```{r ozone2019, echo=FALSE, fig.align='left', fig.height=3, fig.cap="2019 monthly Ozone levels", message=FALSE, warning=FALSE, error=FALSE}
epa_data %>% 
  filter(year(Date) == 2019) %>% 
  ggplot(aes(x = month(Date, label = TRUE), y = Ozone)) +
  geom_boxplot(fill = "red") +
  labs(title = "Monthly Ozone concentrations (2019)", x = "Month", y = expression(Concentration~(mu*g/m^3))) +
  theme_minimal()
```
---
...
https://github.com/esthermy63/EDE_Fall2025
